...
const SHEET_ID = '1SGt-Br7K-pMlfVDVxDEB219hYeBDUC3AEuf_-DviL1s';
...
function createAppointment(sheet, body) {
  ...
  const targetDate = String(body.appointmentDate).trim();
  const targetHour = Number(String(body.appointmentTime).split(':')[0]);

  const values = sheet.getDataRange().getDisplayValues();

  for (let i = 1; i < values.length; i++) {
    const rowDate = String(values[i][1]).trim();
    const rowHour = Number(String(values[i][2]).split(':')[0]);
    const rowStatus = String(values[i][7]).trim().toLowerCase();

    if (rowDate === targetDate && rowHour === targetHour && rowStatus !== 'cancelled') {
      throw new Error('Ese horario ya fue reservado. Selecciona otro horario.');
    }
  }
